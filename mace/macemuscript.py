...
def setup_singlepoint(input_file, model_type='DipoleMACE', model_path=None):
    ...
    start=time.time()
    dipole = get_dipole(atoms, calc)/au_to_ang
    finish=time.time()
    totdipole = np.linalg.norm(dipole)
    print(f"Molecular dipole moment (x,y,z)/au: {dipole[0]} {dipole[1]} {dipole[2]} \n")
    print(f"Total dipole moment au, D: {totdipole} {totdipole*au_to_debye} \n")
...
def calculate_numerical_hessian(atoms, calc, displace=1e-3):
    ...
    for i in range(n_atoms):
        ...
        for j in range(3):
            ...
            dipole_plus = get_dipole(atoms, calc) / au_to_ang            

            # Negative displacement
            atoms.positions[i][j] -= 2 * dx
            #forces_minus = -atoms.get_forces()
            dipole_minus = get_dipole(atoms, calc) / au_to_ang
